...
exports.normalizePokeballId = exports.getPokeballDefinition = exports.isValidPokeballId = exports.POKEBALL_DEFINITIONS = exports.DEFAULT_POKEBALL_ID = void 0;
...
exports.DEFAULT_POKEBALL_ID = 'poke';
exports.POKEBALL_DEFINITIONS = [
...
const POKEBALLS_BY_ID = (() => {
    const map = {};
    for (const ball of exports.POKEBALL_DEFINITIONS) {
        map[ball.id] = ball;
    }
    return map;
})();
function isValidPokeballId(id) {
    if (!id) {
        return false;
    }
    return POKEBALLS_BY_ID[id] !== undefined;
}
exports.isValidPokeballId = isValidPokeballId;
function getPokeballDefinition(id) {
    if (!id) {
        return undefined;
    }
    return POKEBALLS_BY_ID[id];
}
exports.getPokeballDefinition = getPokeballDefinition;
/**
 ...
 */
function normalizePokeballId(id) {
    return isValidPokeballId(id) ? id : exports.DEFAULT_POKEBALL_ID;
}
exports.normalizePokeballId = normalizePokeballId;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.stripEmoji = exports.crystalPaletteAttribute = exports.cardClassName = exports.skinClassName = void 0;
/**
...
/**
 * The `data-crystal-palette` attribute value every Crystal-capable surface
 * (full card, compact Explorer HUD, PokeGear) sets on its own card root -
 * see `[data-crystal-palette="night"]` in media/pokedev-tokens.css for what
 * actually consumes it. Set unconditionally, even under the `pokedev` skin:
 * `.tc-skin-crystal[data-crystal-palette="night"]` only ever matches when
 * BOTH are present, so this is harmless (and simpler than branching per
 * caller) when the skin is not Crystal at all.
 */
function crystalPaletteAttribute(model) {
    return model.crystalPalette;
}
exports.crystalPaletteAttribute = crystalPaletteAttribute;
/**
...
function friendshipXpEventAmount(eventType) {
    switch (eventType) {
        case 'debug-grant':
            return 0;
        // Shopify Dev Actions (below) flow through the exact same Dev Action
        // Friendship amount as the generic four here - no special Shopify
        // Friendship logic, per the feature's own requirement.
        case 'build-success':
        case 'test-success':
...
const pokeballs_1 = __webpack_require__(/*! ../../common/pokeballs */ "./src/common/pokeballs.ts");
...
function renderTrainer(model) {
    ...
    card.setAttribute('data-crystal-palette', (0, card_presentation_1.crystalPaletteAttribute)(model));
    if (!model.connected) {
...
function renderPokemonRow(entry, labels) {
    ...
    // Display-only, per this milestone: no click handler, no selector, no
    // context menu. PokéGear's PARTY tab remains the only place a ball is
    // ever changed - this is purely "what does this Pokemon currently have".
    // Appended last/rightmost deliberately: Pokemon, name/level, XP/
    // friendship and Partner state all outrank the ball in visual priority -
    // see media/explorer.css's matching narrow-width comment for how that
    // ordering plays out as the sidebar shrinks.
    const ball = el('img', 'pd-item-ball');
    ball.setAttribute('src', entry.pokeballSpriteUri);
    ball.setAttribute('alt', '');
    const ballName = (0, pokeballs_1.getPokeballDefinition)(entry.pokeballId)?.name;
    if (ballName) {
        ball.title = ballName;
    }
