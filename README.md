...
A dedicated panel that gathers your Trainer's whole picture into five tabs,
...
- **Party** — your party Pokémon with level and Friendship, a detail panel per
  Pokémon with a **Make Partner** button, and each Pokémon's chosen **Poké
  Ball** — click **Change Ball** to pick from all 38 selectable balls in a
  searchable grid. Purely cosmetic: it never affects catching, XP, or
  anything else, and survives evolution like every other part of a Pokémon's
  identity.
- **Bag** — your evolution stones and their quantities, with a **Use** flow
  that only ever offers Pokémon a given stone can actually evolve.
...
### Poké Ball Sprites
- The 38 selectable Poké Ball cosmetics (PARTY tab, PokéGear) are the actual
  item icons from [msikma/pokesprite](https://github.com/msikma/pokesprite)
  (`items/ball/`), an MIT-licensed sprite database project
- © Nintendo / Creatures Inc. / GAME FREAK inc., used for non-commercial,
  fan project purposes only
