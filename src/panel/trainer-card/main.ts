...
import { getPokeballDefinition } from '../../common/pokeballs';
...
/** Sets `data-crystal-palette` on a just-created card root - see
 * `crystalPaletteAttribute`'s own doc comment for what consumes it. Applied
 * at every one of this bundle's card-creation sites (connected, onboarding,
 * loading, error) so Day/Night resolves correctly in every state, not only
 * once data has loaded. */
function applyCrystalPalette(
  card: HTMLElement,
  model: TrainerCardViewModel,
): void {
  card.setAttribute('data-crystal-palette', crystalPaletteAttribute(model));
}
...
function renderPartySlot(
  ...
  // Bottom-left corner, absolutely positioned (`.tc-party-slot` is already
  // `position: relative`) - small enough to read as a detail, never
  // competing with or overlapping the Pokemon sprite itself.
  const ball = el('img', 'tc-party-ball');
...
function renderPartnerBox(model: TrainerCardViewModel): HTMLElement {
  ...
  const ball = el('img', 'tc-partner-ball-icon');
  ball.setAttribute('src', partner.pokeballSpriteUri);
  ball.setAttribute('alt', '');
  const ballName = getPokeballDefinition(partner.pokeballId)?.name;
  if (ballName) {
    ball.title = ballName;
  }
...
function renderCard(model: TrainerCardViewModel): HTMLElement {
  ...
  applyCrystalPalette(card, model);
