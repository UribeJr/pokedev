...
/**
 * Ball selector state for the Party tab's detail pane. Simpler than the
 * Bag's flow: a ball change is immediate and trivially reversible (pick a
 * different one any time), so there is no confirm step - clicking a cell in
 * the grid both applies it and closes the selector in one action.
 */
let ballSelectorOpen = false;
/** Client-side-only substring filter over `pokeballCatalog` - never sent to
 * the host, the full catalog is already in the pushed view model. */
let ballSearchQuery = '';
...
function activityEntryLabel(entry: PokeGearActivityEntry): string {
  ...
  switch (entry.type) {
    ...
    case 'shopify-theme-check-success':
      return 'Shopify theme check passed';
    case 'shopify-theme-push-success':
      return 'Shopify theme pushed';
    case 'shopify-app-build-success':
      return 'Shopify app built';
    case 'shopify-app-deploy-success':
      return 'Shopify app deployed';
    case 'shopify-extension-deploy-success':
      return 'Shopify extension deployed';
    case 'shopify-function-deploy-success':
      return 'Shopify function deployed';
    case 'shopify-theme-publish-success':
      return 'Shopify theme published';
    case 'debug-grant':
      return 'Debug grant';
...
function renderPartyRow(
  ...
  row.appendChild(pokeballIcon(entry.pokeballSpriteUri, 'pg-party-ball'));
...
/** A small, non-interactive ball sprite - deliberately an `<img>`, not a CSS
 * background: unlike the CSS-drawn placeholder ball used elsewhere for a
 * failed-avatar fallback, every one of the 38 real ball sprites needs its
 * own image. `image-rendering: pixelated` (see media/pokegear.css) keeps it
 * crisp at this small size. */
function pokeballIcon(spriteUri: string, className: string): HTMLElement {
...
function renderPartyTab(model: PokeGearViewModel): HTMLElement {
  ...
  if (selected) {
    ...
    detail.appendChild(
      renderBallSection(selected, party.pokeballCatalog, labels),
    );
...
/**
 * The detail pane's BALL section: current ball sprite + name, a CHANGE BALL
 * button, and (when open) the grid selector. Mirrors the Bag tab's
 * detail-pane-grows-a-sub-flow pattern, but simpler - see `ballSelectorOpen`'s
 * own doc comment for why there's no confirm step here.
 */
function renderBallSection(
  selected: ExplorerPokemonEntry,
  catalog: PokeGearPokeballOption[],
  labels: PokeGearLabels,
): HTMLElement {
  const section = el('div', 'pg-ball-section');
  section.appendChild(sectionTitle(labels.ballLabel));

  const current = el('div', 'pg-ball-current');
  current.appendChild(
    pokeballIcon(selected.pokeballSpriteUri, 'pg-ball-current-icon'),
  );
  const currentBall = catalog.find((b) => b.id === selected.pokeballId);
  current.appendChild(
    el('span', 'tc-party-name', currentBall?.name ?? labels.unknownValue),
  );
  section.appendChild(current);

  if (!ballSelectorOpen) {
    const changeButton = el('button', 'tc-button', labels.changeBallButton);
    changeButton.type = 'button';
    changeButton.addEventListener('click', () => {
      ballSelectorOpen = true;
      rerender();
    });
  ...
  const search = el('input', 'tc-input pg-ball-search') as HTMLInputElement;
  search.type = 'text';
  search.placeholder = labels.searchBallsPlaceholder;
  search.value = ballSearchQuery;
  search.setAttribute('aria-label', labels.searchBallsPlaceholder);
  search.addEventListener('input', () => {
    ballSearchQuery = search.value;
    rerender();
    // `rerender()` rebuilds this input fresh, which drops focus - restore it
    // (and the caret position) so typing a filter doesn't require re-clicking
    // after every keystroke.
    const restored =
      document.querySelector<HTMLInputElement>('.pg-ball-search');
    restored?.focus();
    restored?.setSelectionRange(search.value.length, search.value.length);
  });
  section.appendChild(search);

  const query = ballSearchQuery.trim().toLowerCase();
  const filtered =
    query.length === 0
      ? catalog
      : catalog.filter((ball) => ball.name.toLowerCase().includes(query));

  const grid = el('div', 'pg-ball-grid');
  if (filtered.length === 0) {
    grid.appendChild(el('p', 'tc-devbadges-empty', labels.noBallsFoundLabel));
  } else {
    for (const ball of filtered) {
      grid.appendChild(
        renderBallCell(
          ball,
          ball.id === selected.pokeballId,
          selected.nickname,
        ),
      );
    }
...
function renderBallCell(
  ball: PokeGearPokeballOption,
  isSelected: boolean,
  nickname: string,
): HTMLElement {
  const cell = el(
    'button',
    isSelected ? 'pg-ball-cell pg-ball-cell-selected' : 'pg-ball-cell',
  );
  cell.type = 'button';
  cell.title = ball.name;
  cell.setAttribute('aria-pressed', String(isSelected));
  cell.appendChild(pokeballIcon(ball.spriteUri, 'pg-ball-cell-icon'));
  cell.appendChild(el('span', 'pg-ball-cell-name', ball.name));
  cell.addEventListener('click', () => {
    resetBallSelector();
    post({
      command: 'pokegear/setPokeballId',
      nickname,
      pokeballId: ball.id,
    });
    rerender();
...
function render(model: PokeGearViewModel): void {
  ...
  card.setAttribute('data-crystal-palette', model.crystalPalette);
