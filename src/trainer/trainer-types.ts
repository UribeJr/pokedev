...
export interface PartnerPokemonView {
  ...
  friendship: number;
  /** Same resolved Poké Ball fields as `ExplorerPokemonEntry` - see its
   * identical doc comment in `explorer-types.ts`. */
  pokeballId: string;
...
export interface TrainerCardViewModel {
  ...
  /**
   * The resolved Crystal palette (`'day'`/`'night'`) - only meaningful when
   * `style === 'crystal'`, but always present so the webview never has to
   * guess a fallback. Resolved host-side from `pokedev.crystalPalette`
   * (`'auto'`/`'day'`/`'night'`) against the live VS Code/Cursor appearance
   * when the preference is `'auto'` - see `src/common/crystal-palette.ts`.
   */
