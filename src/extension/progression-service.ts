...
export class ProgressionService {
    ...
    if (grant.isPartner) {
      ...
      toastHub.notifyXp(
        ...
        event.type === 'git-commit' ||
          ...
          isShopifyShipTierEvent(event.type)
          ? 'large'
          : 'normal',
        now,
      );
  ...
  private _reactToEvent(event: ProgressionEvent, pokemonId: string): void {
    ...
    } else if (isShopifyShipTierEvent(event.type)) {
      // SHIP tier: a real push/deploy/publish reached a store or platform -
      // the same full "celebrate" reaction a build/test pass gets, reusing
      // `notifyDevAction` rather than new reaction infrastructure.
      reactionHub.notifyDevAction(pokemonId, false);
    } else if (isShopifyVerifyTierEvent(event.type)) {
      // VERIFY tier: a check/build confirmed something, without shipping it
      // anywhere - the same subtle "notice" a typecheck/lint pass gets.
      reactionHub.notifyDevAction(pokemonId, true);
    }
  }
...
/**
 * SHIP-tier Shopify outcomes: something actually reached a store or
 * platform. Gets the same "large"/celebrate emphasis as a build, test pass
 * or commit - see both call sites above.
 */
function isShopifyShipTierEvent(type: ProgressionEvent['type']): boolean {
  return (
    type === 'shopify-theme-push-success' ||
    type === 'shopify-app-deploy-success' ||
    type === 'shopify-extension-deploy-success' ||
    type === 'shopify-function-deploy-success' ||
    type === 'shopify-theme-publish-success'
  );
}

/**
 * VERIFY-tier Shopify outcomes: confirmed something without shipping it
 * anywhere. Gets the same subtle emphasis as a typecheck/lint pass.
 */
function isShopifyVerifyTierEvent(type: ProgressionEvent['type']): boolean {
  return (
    type === 'shopify-theme-check-success' ||
    type === 'shopify-app-build-success'
  );
}
