...
export class DailyChallengesService implements vscode.Disposable {
  ...
  private async _buildEligibilityContext(
    ...
    // The only awaited steps: a one-shot `vscode.tasks.fetchTasks()` scan and
    // a one-shot file-existence scan, never polled and only ever run at
    // generation time - see their own doc comments in
    // `dev-action-capabilities.ts`/`shopify-project-detection.ts`.
    const [devActionCapabilities, shopifyCapabilities] = await Promise.all([
      detectDevActionCapabilities(),
      detectShopifyCapabilities(),
    ]);
