...
export type DailyChallengeSignal =
  ...
  | { kind: 'shopify-dev-action'; shopifyActionType: ShopifyDevActionType };
...
function applySignalToInstance(
  ...
  switch (instance.eventType) {
    ...
    case 'shopify-theme-publish-success':
      return signal.kind === 'shopify-dev-action' &&
        signal.shopifyActionType === instance.eventType
        ? addProgress(instance, 1)
        : instance;

    case 'shopify-dev-action-any':
      return signal.kind === 'shopify-dev-action'
        ? addProgress(instance, 1)
        : instance;
  }
