...
export const DAILY_CHALLENGE_CATALOG: readonly DailyChallengeDefinition[] = [
  ...
  /* -------------------------- shopify dev actions --------------------------- */
  // Same reasoning as the generic Dev Action challenges just above: filed
  // under 'coding'/'wildcard' rather than a new category, and never offered
  // at all unless `shopify-project-detection.ts` found evidence this
  // workspace is actually a Shopify theme or app project - see
  // `hasShopifyCapability` below.
  {
    id: 'theme-doctor',
  ...
  {
    id: 'ship-the-theme',
    family: 'shopify-theme-push-count',
    category: 'coding',
    eventType: 'shopify-theme-push-success',
    requiresShopifyCapability: 'theme',
    title: 'Ship The Theme',
    description: 'Complete 1 successful Shopify theme push',
    tiers: [{ minTrainerLevel: 1, target: 1, rewardTrainerXp: 30 }],
  },
  {
    id: 'app-builder',
    family: 'shopify-app-build-count',
    category: 'coding',
    eventType: 'shopify-app-build-success',
    requiresShopifyCapability: 'app',
    title: 'App Builder',
    description: 'Complete 2 successful Shopify app builds',
    tiers: [{ minTrainerLevel: 1, target: 2, rewardTrainerXp: 25 }],
  },
  {
    id: 'deploy-day',
    ...
    requiresShopifyCapability: 'app',
    title: 'Deploy Day',
    description: 'Complete 1 successful Shopify app deploy',
    tiers: [{ minTrainerLevel: 1, target: 1, rewardTrainerXp: 35 }],
  },
  {
    id: 'storefront-sprint',
    family: 'shopify-dev-action-any-count',
    category: 'wildcard',
    eventType: 'shopify-dev-action-any',
    requiresShopifyCapability: 'any',
    title: 'Storefront Sprint',
    description: 'Complete 3 accepted Shopify Dev Actions',
    tiers: [{ minTrainerLevel: 1, target: 3, rewardTrainerXp: 30 }],
  },
];
...
export function isDefinitionEligible(
  ...
  if (
    definition.requiresShopifyCapability &&
    !hasShopifyCapability(definition.requiresShopifyCapability, context)
  ) {
    return false;
  }
  return true;
...
function hasShopifyCapability(
  requirement: ShopifyCapabilityRequirement,
  context: DailyChallengeEligibilityContext,
): boolean {
  const caps = context.shopifyCapabilities;
  switch (requirement) {
    case 'theme':
      return caps.theme;
    case 'app':
