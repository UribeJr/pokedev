...
import { ShopifyCapabilities } from '../progression/shopify-dev-action-types';
...
/** Which Shopify project capability a definition needs - see
 * `isDefinitionEligible` and `extension/shopify-project-detection.ts`.
 * `'any'` means "looks like a theme project OR an app project". */
export type ShopifyCapabilityRequirement = 'theme' | 'app' | 'any';
...
export type DailyChallengeEventType =
  ...
  | 'dev-action-any'
  // Shopify Dev Actions - each maps 1:1 to the identically-named
  // `ProgressionEventType`/`ShopifyDevActionType`, translated by
  // `daily-challenges-service.ts` exactly like every other signal here.
  ...
  /** Any accepted Shopify Dev Action - "Storefront Sprint". */
  | 'shopify-dev-action-any';
...
export interface DailyChallengeDefinition {
  ...
  /** Requires a specific Shopify project capability - see
   * `isDefinitionEligible` and `ShopifyCapabilityRequirement`. */
  requiresShopifyCapability?: ShopifyCapabilityRequirement;
...
export interface DailyChallengeEligibilityContext {
  ...
  /** What kind of Shopify project this workspace looks like, from
   * lightweight file-existence checks - see
   * `extension/shopify-project-detection.ts`. */
  shopifyCapabilities: ShopifyCapabilities;
}
