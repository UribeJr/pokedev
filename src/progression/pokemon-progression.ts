...
import { DEFAULT_POKEBALL_ID, normalizePokeballId } from '../common/pokeballs';
...
export function createDefaultPokemonProgress(
  ...
  return {
    ...
    pokeballId: DEFAULT_POKEBALL_ID,
  };
}
...
export function normalizePokemonProgress(
  ...
  return {
    ...
    friendship: isFiniteNumber(raw['friendship'])
      ...
      : DEFAULT_POKEMON_FRIENDSHIP,
    // Same migration shape as `friendship` above: a record saved before Poké
    // Ball customization existed (or hand-edited to something no longer in
    // the catalog) simply has no usable value here, and normalizes to the
    // standard Poké Ball rather than throwing or leaving it undefined.
    pokeballId: normalizePokeballId(
      typeof raw['pokeballId'] === 'string' ? raw['pokeballId'] : undefined,
    ),
  };
}
