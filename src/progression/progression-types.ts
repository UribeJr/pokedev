 ...
 */
import { PokeballId } from '../common/pokeballs';
...
export type ProgressionEventType =
  ...
  | 'typecheck-success'
  | 'lint-success'
  // Shopify Dev Actions: the same "a task ended" signal, classified by
  // `progression/shopify-dev-action-classifier.ts` into a verified Shopify
  // CLI outcome (a theme check, a theme push, an app build/deploy, ...) -
  // see that module's own doc comment and `ShopifyDevActionType`. Kept as a
  // separate imported union rather than merged into the six literals above,
  // since Shopify outcomes have their own classifier and cooldown table.
  | ShopifyDevActionType
  | 'debug-grant'
...
export interface PokemonProgress {
  ...
  friendship: number;
  /**
   * Cosmetic Poké Ball customization - see `common/pokeballs.ts`. Purely
   * visual: never affects XP, Friendship, evolution eligibility, or
   * anything else. Survives evolution for free, exactly like `friendship` -
   * both live on the same nickname-keyed record. Every Pokemon saved before
   * this existed simply has no field here; `normalizePokemonProgress`
   * defaults it to the standard Poké Ball on read, the same migration
   * pattern already used for `friendship`.
   */
