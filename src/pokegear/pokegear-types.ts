 ...
 * elsewhere. It DOES read the resolved Crystal Day/Night palette
 * (`crystalPalette` below) - that preference is shared by every
 * Crystal-capable surface, PokeGear included, even though the Trainer Card
 * STYLE preference itself is not.
 */
...
/** One selectable Poké Ball cosmetic, resolved for display - a
 * `PokeballDefinition` (`common/pokeballs.ts`) plus its webview-safe sprite.
 * Built host-side, exactly like `TrainerSpriteOption` - the webview never
 * resolves `assetPath` itself. */
export interface PokeGearPokeballOption {
  id: string;
  name: string;
  spriteUri: string;
}
...
export interface PokeGearPartyView {
   ...
   * the Explorer team list - up to `PARTY_SLOTS`. Each entry already
   * carries its own resolved `pokeballId`/`pokeballSpriteUri` - see
   * `ExplorerPokemonEntry`'s own doc comment in `explorer-types.ts`. */
  ...
  /** The full ball selector catalog, in canonical (sort) order - sent once
   * per push rather than per Pokemon, since it never varies by which
   * Pokemon is selected. */
  pokeballCatalog: PokeGearPokeballOption[];
}
...
export interface PokeGearViewModel {
  ...
  /** Resolved Crystal Day/Night palette - see the module doc comment above.
   * `TrainerCardViewModel.crystalPalette` in `trainer-types.ts` documents the
   * resolution itself. */
