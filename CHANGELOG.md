...
- feat: Shopify Dev Actions — the existing Dev Actions system now also
  recognizes verified Shopify CLI outcomes: a passing `shopify theme check`,
  a successful `theme push`, an `app build`, or an `app deploy` each earn
  modest Trainer/partner Pokémon XP and count toward Friendship exactly like
  a build or test pass. `theme dev`/`app dev`/`pull`/`auth login`/`version`/
  `help`/`list`/`info` never earn anything — only a real, verified outcome
  does
- feat: `PokéDev: Shopify Actions...` — a capability-aware QuickPick (grouped
  THEME/APP) that runs Theme Check, Theme Push, App Build or App Deploy
  through a real, PokéDev-managed VS Code task, so success is reliably
  observed the same way a hand-authored task already is. Also exposed as four
  standalone commands. Detection never scrapes terminal text or history,
  never reads source, and never touches Shopify credentials or shop
  domains — only a task's own exit code and declared metadata, or a
  lightweight file-existence check (`shopify.app.toml`, `layout/
  theme.liquid`, etc.) to decide whether this looks like a Shopify theme or
  app project at all
- feat: five new Daily Challenges — Theme Doctor, Ship The Theme, App
  Builder, Deploy Day and Storefront Sprint — offered only in a workspace
  that actually looks like the right kind of Shopify project
...
- chore: `pokedev.inventory` and `pokedev.claimedItemRewards` join the
  ...
  machines
- feat: Crystal Day/Night — the Crystal Trainer Card style now has a dark
  palette variant to match a dark VS Code/Cursor theme, in addition to the
  original cream Crystal Day look. `pokedev.crystalPalette` (`auto`/`day`/
  `night`, default `auto`) controls it, with a `PokéDev: Change Crystal
  Palette` command; Auto follows the IDE's own light/dark appearance live,
  no reload needed. The same resolved palette applies everywhere Crystal
  renders — the full Trainer Card, the compact Explorer Trainer view, and
  PokéGear (which always uses Crystal) — never a separate preference per
  surface
- fix: removed the decorative corner-fold/corner-spiral accents from the
  Crystal Trainer Card and PokéGear — plain square pixel borders on every
  corner now, matching the rest of the skin
- feat: persistent, per-Pokémon Poké Ball customization — every Pokémon can
  now be assigned any of 38 real, full-color Poké Ball sprites (Poké/Great/
  Ultra/Master through Legends: Arceus specialty and Hisuian variants) from
  PokéGear's PARTY tab. Purely cosmetic: it is never an inventory, nothing is
  bought/caught/consumed, and it grants no XP — just a visual choice that
  persists with the Pokémon and survives evolution exactly like its
  nickname, level, or Friendship. Shows up as a small sprite on PokéGear
  Party rows, the full Trainer Card's party grid and Partner panel, and now
  the Explorer sidebar's PARTY view too — display-only there, with PokéGear
  remaining the one place a ball is ever changed
